...
describe Timesheet do
    describe '#date' do
        it 'returns a valid date from user input' do
            ...
            expect(Timesheet.date('start')).to eq (Date.parse('2022-03-26'))
        end
        it 'returns InvalidDateError if date outside current week' do
            allow(Timesheet).to receive(:gets).and_return('20/03.2022')
            expect { Timesheet.date('start') }.to raise_error(InvalidDateError)
        end
    ...
    describe '#time' do
        it 'returns a valid Time instance from user input' do
            ...
            expect(Timesheet.time('start', Date.parse('2022-03-26'))).to be_an_instance_of Time
        end
        it 'returns InvalidTimeError if user\'s input cannot be converted to Time' do
            ...
            expect { Timesheet.time('start', Date.parse('2022-03-26')) }.to raise_error(InvalidTimeError)
        end
